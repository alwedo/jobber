...
const getAndUpdateQuery = `-- name: GetAndUpdateQuery :one
UPDATE queries
SET
...
type GetAndUpdateQueryParams struct {
	Keywords string
	Location string
}

func (q *Queries) GetAndUpdateQuery(ctx context.Context, arg *GetAndUpdateQueryParams) (*Query, error) {
	row := q.db.QueryRow(ctx, getAndUpdateQuery, arg.Keywords, arg.Location)
	var i Query
	err := row.Scan(
		&i.ID,
		&i.Keywords,
